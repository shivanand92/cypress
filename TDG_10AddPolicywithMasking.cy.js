describe('Add Policy With Masking Automation', () => {
  let credentials;
  ...
  it('Creates a new policy using values from userCredentials.json', function () {
    ...
    //========================Group and Sub-group Select=================================
    cy.wait(1000);
    cy.contains('button', 'Group').click({ force: true });
    cy.get('input[type="text"]').eq(2).type(credentials.policy.GroupName);
    cy.get('input[type="checkbox"]').eq(1).check({ force: true });
    cy.wait(1000);
    cy.contains('button', 'Sub-group').click({ force: true });

    cy.get('input[type="text"]').eq(2).type(credentials.policy.SubgroupName);
    cy.get('input[type="checkbox"]').eq(1).check({ force: true });
    cy.contains('button', 'Next').click();
    //===================================Selected===================================
    cy.wait(1000);
