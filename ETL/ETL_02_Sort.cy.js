...
describe('Sort', () => {
  ...
  let data;
  let sortData; // ✅ FIX 1: declare sortData
  ...
  before(() => {
    ...
    cy.fixture('etl/sort').then((fixtureData) => {
      sortData = fixtureData; // ✅ FIX 2: load sort.json
    });
  });
...
cy.wait(2000);
  ...
  //====================================
    ...
    // ================= SORT TRANSFORMATION (FIXED) =================

    // Operator
    cy.get('select.w-full.p-2.text-sm')
      .should('be.visible')
      .select(sortData.operator);
