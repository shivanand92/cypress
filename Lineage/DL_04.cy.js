...
//========================================================Schema node clicking==================================================
cy.get('div[title]')
  .filter((_, el) => el.innerText.trim() !== '')   // ensure it has visible text
  .first()
  .click({ force: true });


//==================================================Asset information =====================================================
  cy.get('button[title="Asset Information"]', { timeout: 10000 })
  .should('be.visible')
  .click({ force: true });

  cy.screenshot('information displayed successfully');

cy.get('button.text-gray-500.hover\\:text-gray-700.text-lg.font-semibold', { timeout: 10000 })
  .should('exist')
  .click({ force: true });

  //=============================================Show forward connections=================================================

  cy.get('button[title="Show Forward Connections"]', { timeout: 10000 })
  .should('exist')
  .click({ force: true });
...
cy.get('button[title="Close Forward Connections"]', { timeout: 10000 })
  .should('exist')
  .click({ force: true });

cy.wait(1000);
  //===========================================Show All asset connections=============================================
  cy.get('button[title="Show All Asset Connections"]').click();
cy.wait(1000);
cy.get('button[title="Close All Connections"]').click();
//=============================================close all assset connection==========================================
cy.wait(1000);
cy.get('button[title="Column Level Details not available"]')
  .invoke('attr', 'title')
  .then((tooltip) => {
    cy.log("Tooltip message: " + tooltip);
    expect(tooltip).to.eq("Column Level Details not available");
  });

cy.wait(1000);
  cy.get('button.absolute.-top-3.-right-3.w-6.h-6.bg-red-500.rounded-full.text-white.flex.items-center.justify-center.text-xs.shadow-lg')
  .click();
