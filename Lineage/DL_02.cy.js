...
describe('Lineage Homepage1Lineage Homepage1 selecting Multiple Database,connection,schema,Type', () => {
...
cy.get('button[title="Back"]').click();
//===================================================================novigation over==========================================
...
cy.screenshot('Visualize button is enabled successfully');
cy.wait(1000);
cy.get('button[title="Remove from Lineage"]').click();

cy.get('input[placeholder="Search in table..."]').clear();
cy.wait(1000);
cy.get('div.overflow-x-auto').scrollTo('left', { duration: 800 });
cy.contains('th', 'Database').click();
cy.wait(1000);
cy.contains('th', 'Database').click();
