...
describe('Data-Quality Rules Creation with Pre-built and Data  with Multiple Groups, Connections,Tables', () => {
  let credentials;
...
//  first group 
    ...
    cy.get('button.border-2.rounded-lg.cursor-pointer').eq(0).click();
    cy.wait(1000);
  ...
  //===================================================================================================
    ...
    //==============================================================================================
    cy.get('button.p-2.rounded.hover\\:bg-gray-200.text-gray-700').click();  // second page
    // Step 8: Interact with first dropdown
    /*cy.get('.react-select__dropdown-indicator').eq(0).click();
    cy.get('input.react-select__input').type('oracleforautomation');
    cy.wait(1000);*/
    ...
    cy.get('.react-select__dropdown-indicator').eq(0).click().then($indicator => {
      ...
      cy.wrap($indicator)
        .closest('.react-select__control')
        .find('input.react-select__input')
        .type(credentials.Dataquality.connection1);
      // Now select the option
      cy.get('.react-select__option').contains(credentials.Dataquality.connection1).click();
    });
    ...
    cy.get('.react-select__dropdown-indicator').eq(1).click().then($indicator => {
      ...
      cy.wrap($indicator)
        .closest('.react-select__control')
        .find('input.react-select__input')
        .type(credentials.Dataquality.shchema1);
      // Now select the option (checkbox or item)
      cy.get('.react-select__option').contains(credentials.Dataquality.shchema1).click();
    });
   ...
   // Open the third dropdown (index 2, since it's zero-based)
    cy.get('.react-select__dropdown-indicator').eq(2).click().then($indicator => {
      ...
      cy.wrap($indicator)
        .closest('.react-select__control')
        .find('input.react-select__input')
        .type(credentials.Dataquality.table1);
      // Now select the option (checkbox or item)
      cy.get('.react-select__option').contains(credentials.Dataquality.table1).click();
    });

    /*// Open the third dropdown (index 2, since it's zero-based)
    // Select all options from the "Select Table" react-select (multi)
    cy.contains('div', 'Select Table')
      .parents('.react-select__control')
      .find('.react-select__dropdown-indicator')
      .click();
