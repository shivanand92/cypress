...
describe('Data-Quality Rules Creation with Pre-built and Data with Single Groups - Single connections - Multiple Tables and Schemas', () => {
  let credentials;
  ...
  it('Logs into Tantor portal and creates multiple groups', function () {
    ...
    // Step 8: Interact with first dropdown - select all options robustly
    cy.get('.react-select__dropdown-indicator').eq(0).click();
    cy.get('.react-select__menu').should('be.visible');
    cy.get('.react-select__menu .react-select__option').then(($opts) => {
      const optionLabels = [...$opts].map((opt) => opt.textContent.trim());
      cy.wrap(optionLabels).each((label) => {
        // Re-open the menu if it closed (common with single-select)
        ...
        cy.contains('.react-select__menu .react-select__option', label)
          .scrollIntoView()
          .click({ force: true });
      });
    });
